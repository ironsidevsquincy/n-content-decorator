...
module.exports = (content, options) => {
	...
	if (!content.metadata && (content.primaryTheme || content.primarySection)) {
		...
		//HACK uses qraphql's logic for working out branding which replicates primary brand
		if (content.branding) content.branding.primary = 'brand';
		primaryMetadata = [content.primaryTheme, content.primarySection, content.branding];
	}
